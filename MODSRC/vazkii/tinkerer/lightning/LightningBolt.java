...
public class LightningBolt {
	...
	private void calculateCollisionAndDiffs() {
		...
		for (Segment segment : segments) {
			...
			{
				lastactivesegment.put(lastsplitcalc, lastactiveseg);// put last
																	// active
																	// segment
																	// for split
																	// in
				...
				lastactiveseg = lastactivesegment.get(splitparents.get(segment.splitno));// last
																							// active
																							// is
																							// parent
				splitresistance = lastactiveseg < segment.segmentno ? 50 : 0;// already
																				// teminated
																				// if
																				// the
																				// last
		...
		lastactivesegment.put(lastsplitcalc, lastactiveseg);// put last active
															// segment for split
															// in map

		lastsplitcalc = 0;
		lastactiveseg = lastactivesegment.get(0);
		for (Iterator<Segment> iterator = segments.iterator(); iterator.hasNext();)// iterate
																					// each
																					// segment
																					// and
																					// kill
	...
	public void onUpdate() {
		...
		// bbTestEntityDamage(); These bolts don't cause damage, this feature is
		// redundant
