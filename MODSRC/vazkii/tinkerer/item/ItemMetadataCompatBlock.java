...
public class ItemMetadataCompatBlock extends ItemBlock {
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ, int metadata) {
       if (!world.setBlockAndMetadataWithNotify(x, y, z, this.blockID2, stack.getItemDamage()))
               return false;

       if (world.getBlockId(x, y, z) == this.blockID2){
           Block.blocksList[this.blockID2].onBlockPlacedBy(world, x, y, z, player);
           Block.blocksList[this.blockID2].func_85105_g(world, x, y, z, metadata);
       }
